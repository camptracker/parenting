...
import { disagreeSections } from './disagreeData'
import type { DisagreeSection } from './disagreeData'

function DisagreeCard({ topic, isOpen, onToggle }: { topic: DisagreeSection; isOpen: boolean; onToggle: () => void }) {
...
export default function DisagreeSection() {
  ...
  return (
    <main>
      <section>
        <h3 className="section-label" style={{ color: '#5b8a72' }}>6 Dimensions of Healthy Disagreement</h3>
        {disagreeSections.map(t => (
          <DisagreeCard key={t.id} topic={t} isOpen={openCards.has(t.id)} onToggle={() => toggleCard(t.id)} />
        ))}
      </section>
    </main>
  )
