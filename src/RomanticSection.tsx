...
import { romanticTopics } from './romanticData'
import type { RomanticTopic } from './romanticData'

function RomanticCard({ topic, isOpen, onToggle }: { topic: RomanticTopic; isOpen: boolean; onToggle: () => void }) {
...
export default function RomanticSection() {
  ...
  return (
    <main>
      <section>
        <h3 className="section-label" style={{ color: '#c44060' }}>9 Dimensions of Romance</h3>
        {romanticTopics.map(t => (
          <RomanticCard key={t.id} topic={t} isOpen={openCards.has(t.id)} onToggle={() => toggleCard(t.id)} />
        ))}
      </section>
    </main>
  )
