...
import { friendshipTopics } from './friendshipData'
import type { FriendshipTopic } from './friendshipData'

function FriendshipCard({ topic, isOpen, onToggle }: { topic: FriendshipTopic; isOpen: boolean; onToggle: () => void }) {
...
export default function FriendshipSection() {
  ...
  return (
    <main>
      <section>
        <h3 className="section-label" style={{ color: '#b07040' }}>7 Dimensions of Friendship</h3>
        {friendshipTopics.map(t => (
          <FriendshipCard key={t.id} topic={t} isOpen={openCards.has(t.id)} onToggle={() => toggleCard(t.id)} />
        ))}
      </section>
    </main>
  )
